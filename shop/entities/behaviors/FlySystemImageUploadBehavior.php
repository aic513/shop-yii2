...
class FlySystemImageUploadBehavior extends ImageUploadBehavior
{
    ...
    public function afterSave(): void
    {
        if ($this->file instanceof UploadedFile) {
            ...
            $this->filesystem->createDirectory(pathinfo($path, PATHINFO_DIRNAME));
            $this->filesystem->move($path, file_get_contents($this->file->tempName));
    ...
    public function createThumbs(): void
    {
        ...
        foreach ($this->thumbs as $profile => $config) {
            ...
            if ($this->filesystem->fileExists($path) && !$this->filesystem->fileExists($thumbPath)) {
                ...
                $thumb->save($tmpPath);
                $this->filesystem->createDirectory(pathinfo($thumbPath, PATHINFO_DIRNAME));
                $this->filesystem->move($thumbPath, file_get_contents($tmpPath));
            }
        }
    }
